...
public class PlanckDimension implements Dimension{
	...
	public PlanckDimension(String dimensionIn) {
		...
		for (int i=0; i<results.size(); i++) {
			...
			}else if (results.get(i).indexOf('Q')!= -1) {
				if(results.get(i).indexOf('-')!= -1) {
					Q = - Character.getNumericValue(results.get(i).charAt(9));
				} else {
					Q = Character.getNumericValue(results.get(i).charAt(8));
				}
	...
	public void setQ(int dimensionIn) {
		Q = dimensionIn;
	}
	...
	public boolean equals(Object otherDimension) {
		...
		PlanckDimension otherDimensionCast = (PlanckDimension) otherDimension;
		...
		}else if(Q!=otherDimensionCast.getQ()) {
			return false;
